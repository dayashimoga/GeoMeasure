      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Calculated Area: ${result.area.toStringAsFixed(2)} ${result.areaUnit.name}'),
            Text('Perimeter / Boundary: ${result.perimeter.toStringAsFixed(2)} ${result.distanceUnit.name}'),
            if (result.volume > 0)
              Text('Volume: ${result.volume.toStringAsFixed(2)} m³'),
            const SizedBox(height: 8),
            Text('Estimated Accuracy: ${result.estimatedAccuracyPercentage.toStringAsFixed(1)}%'),
          ],
        ),
      ),
    );
